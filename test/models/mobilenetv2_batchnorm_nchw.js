...
describe('test mobilenetv2 batchnorm nchw', function() {
  ...
  before(async () => {
    ...
    async function buildConvBatchNorm(
      ...
      const variance =
          await utils.buildConstantFromNpy(device, builder, new URL(varianceName, url));
    ...
    async function buildMobileNet() {
      ...
      const conv0Weight = await utils.buildConstantFromNpy(device, 
  ...
  async function testMobileNetV2(graph, inputFile, expectedFile) {
    const inputs = {
      'input': {resource: await utils.createGPUBufferFromNpy(device, new URL(inputFile, url))},
    };
    const outputs = {
      'reshape0': {resource: await utils.createGPUBuffer(device, utils.sizeOfShape([1, 1000]))},
    };
    graph.compute(inputs, outputs);
    const expected =
        await utils.createTypedArrayFromNpy(new URL(expectedFile, url));
    utils.checkValue(await utils.readbackGPUBuffer(device, utils.sizeOfShape([1, 1000]), outputs.reshape0.resource), expected, utils.modelFp32AccuracyCriteria);
  }
