...
def PrintInputResource(oprand, operation, opInsList, value, test):
    typedArray = oprand.type.mappingTypedArrayType
    opValue = GetOperandValue(oprand, operation, opInsList, value)
    IndentedPrint('const %sResource = {resource: await utils.createGPUBuffer(device, utils.sizeOfShape(%s), %s)};' % (oprand, oprand.type.dimensions, opValue),
                  indent=4, file=test)
...
def PrintConstant(oprand, operation, opInsList, opInsInfoList, layout,
    ...
    if operation == 'INSTANCE_NORMALIZATION' and size != None:
        operand = "const %s = builder.constant(%s, {resource: await utils.createGPUBuffer(device, utils.sizeOfShape(%s), %s)});" % \
              (oprand, opDesc, oprand.type.dimensions, opValue * size)
    else:
        operand = "const %s = builder.constant(%s, {resource: await utils.createGPUBuffer(device, utils.sizeOfShape(%s), %s)});" % \
              (oprand, opDesc, oprand.type.dimensions, opValue)
    IndentedPrint(operand, indent=4, file=test)
    ...
    for inputFeedDict, outputFeedDict in example.feedDicts:
            ...
            if mappingParamIndex != -1:
                ...
                if rule == md.MappingRule.OPERAND_OPERAND:
                    PrintInputOperand(op, nnapiOp, curOpInsList, nnapiOpInsList,
                                      layout, test, fused)
                    PrintInputResource(op, nnapiOp, curOpInsList,
                                   inputFeedDict[op], test)
                    computeParamsList.append("'%s': %sResource" % \
                                             (op, op))
        ...
        if len(curOutputsList) == 1:
            IndentedPrint("const graph = builder.build({%s});" % outputOp,
                          indent=4, file=test)
            IndentedPrint(
                "const outputs = {%s: {resource: await utils.createGPUBuffer(device, utils.sizeOfShape(%s))}};" % \
                (outputOp, outputOp.type.dimensions),
                indent=4, file=test)                         
        elif len(curOutputsList) > 1:
            ...
            outputNameBufferList = \
                ["%s: {resource: await utils.createGPUBuffer(device, utils.sizeOfShape(%s))}" % (item.name, item.type.dimensions) for item in outputOp]
        ...
        if len(curOutputsList) == 1:
            IndentedPrint(
                "utils.checkValue(await utils.readbackGPUBuffer(device, utils.sizeOfShape(%s), outputs.%s.resource), expected, %s);" % \
                (outputOp.type.dimensions, outputOp, criteria), indent=4, file=test)
        elif len(curOutputsList) > 1:
            IndentedPrint('for (let i = 0; i < %d; i++) {' % \
                          len(curOutputsList), indent=4, file=test)
            dataStr = 'await utils.readbackGPUBuffer(device, utils.sizeOfShape(%s), outputs[%s[i]].resource)' % (outputOp[0].type.dimensions, ['%s' % k for k in outputOp])
            IndentedPrint(
