...
import * as utils from '%sutils.js';\n""" % ''.join(['../']*detph)
    ...
    IndentedPrint("let device;", indent=2, file=test)
    IndentedPrint("let context;", indent=2, file=test)
    IndentedPrint("before(async () => {", indent=2, file=test)
    IndentedPrint("const adaptor = await navigator.gpu.requestAdapter();", indent=4, file=test)
    IndentedPrint("device = await adaptor.requestDevice();", indent=4, file=test)
    IndentedPrint("context = navigator.ml.createContext(device);", indent=4, file=test) 
