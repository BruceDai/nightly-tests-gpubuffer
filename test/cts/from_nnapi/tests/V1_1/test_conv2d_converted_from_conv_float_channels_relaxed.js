...
describe('CTS converted from NNAPI CTS', function() {
  ...
  it('test conv2d (fused ops) converted from conv_float_channels_relaxed test', async function() {
    ...
    const op2 = builder.constant({type: 'float32', dimensions: [3, 1, 1, 3]}, {resource: await utils.createGPUBuffer(device, utils.sizeOfShape([3, 1, 1, 3]), [1.0, 1.0, 1.0, 2.0, 2.0, 2.0, 3.0, 3.0, 3.0])});
    ...
    utils.checkValue(await utils.readbackGPUBuffer(device, utils.sizeOfShape([1, 1, 1, 3]), outputs.op4.resource), expected, utils.ctsFp32RelaxedAccuracyCriteria);
  });
});
/* eslint-disable max-len */
