...
describe('CTS converted from NNAPI CTS', function() {
  ...
  it('test conv2d (fused ops) converted from depthwise_conv2d_float_large_2_weights_as_inputs_relaxed test', async function() {
    ...
    const op2Resource = {resource: await utils.createGPUBuffer(device, utils.sizeOfShape([1, 2, 2, 4]), [0.25, 0, 10, 100, 0.25, 1, 20, 100, 0.25, 0, 30, 100, 0.25, 1, 40, 100])};
    const op3 = builder.input('op3', {type: 'float32', dimensions: [4]});
    const op3Resource = {resource: await utils.createGPUBuffer(device, utils.sizeOfShape([4]), [6000, 7000, 8000, 9000])};
    const pad0 = 0;
    const stride = 1;
