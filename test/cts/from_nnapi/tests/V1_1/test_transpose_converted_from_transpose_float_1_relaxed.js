...
describe('CTS converted from NNAPI CTS', function() {
  ...
  it('test transpose converted from transpose_float_1_relaxed test', async function() {
    ...
    const outputs = {output: {resource: await utils.createGPUBuffer(device, utils.sizeOfShape([4, 2, 3, 5]))}};
    graph.compute({'input': inputResource}, outputs);
    utils.checkValue(await utils.readbackGPUBuffer(device, utils.sizeOfShape([4, 2, 3, 5]), outputs.output.resource), expected, utils.ctsFp32RelaxedAccuracyCriteria);
  });
});
/* eslint-disable max-len */
