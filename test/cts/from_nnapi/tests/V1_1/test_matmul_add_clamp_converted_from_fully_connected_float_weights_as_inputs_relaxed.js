...
describe('CTS converted from NNAPI CTS', function() {
  ...
  it('test matmul + add + clamp converted from fully_connected_float_weights_as_inputs_relaxed test', async function() {
    ...
    const op2Resource = {resource: await utils.createGPUBuffer(device, utils.sizeOfShape([1, 1]), [2])};
    const b0 = builder.input('b0', {type: 'float32', dimensions: [1]});
    const b0Resource = {resource: await utils.createGPUBuffer(device, utils.sizeOfShape([1]), [4])};
