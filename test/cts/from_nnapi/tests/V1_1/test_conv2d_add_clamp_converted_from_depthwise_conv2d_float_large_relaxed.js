...
describe('CTS converted from NNAPI CTS', function() {
  ...
  it('test conv2d + add + clamp converted from depthwise_conv2d_float_large_relaxed test', async function() {
    ...
    const op2 = builder.constant({type: 'float32', dimensions: [1, 2, 2, 2]}, {resource: await utils.createGPUBuffer(device, utils.sizeOfShape([1, 2, 2, 2]), [0.25, 0, 0.25, 1, 0.25, 0, 0.25, 1])});
    const op3 = builder.constant({type: 'float32', dimensions: [2]}, {resource: await utils.createGPUBuffer(device, utils.sizeOfShape([2]), [100, 200])});
    const pad0 = 0;
    const stride = 1;
    ...
    utils.checkValue(await utils.readbackGPUBuffer(device, utils.sizeOfShape([1, 1, 1, 2]), outputs.op4.resource), expected, utils.ctsFp32RelaxedAccuracyCriteria);
  });
});
/* eslint-disable max-len */
