...
describe('test pow', function() {
  ...
  async function testSqrt(input, expected, shape) {
    ...
    const y = builder.constant(
        {type: 'float32', dimensions: [1]}, {resource: await utils.createGPUBuffer(device, utils.sizeOfShape([1]), [0.5])});
    const z = builder.pow(x, y);
    const graph = builder.build({z});
    const inputs = {'x': {resource: await utils.createGPUBuffer(device, utils.sizeOfShape(shape), input)}};
    const outputs = {'z': {resource: await utils.createGPUBuffer(device, utils.sizeOfShape(shape))}};
    graph.compute(inputs, outputs);
    utils.checkValue(await utils.readbackGPUBuffer(device, utils.sizeOfShape(shape), outputs.z.resource), expected);
  }
  ...
  it('pow 1d', async function() {
    ...
    const y = builder.constant(
        {type: 'float32', dimensions: [3]}, {resource: await utils.createGPUBuffer(device, utils.sizeOfShape([3]), [4, 5, 6])});
    const z = builder.pow(x, y);
    const graph = builder.build({z});
    const inputs = {'x': {resource: await utils.createGPUBuffer(device, utils.sizeOfShape([3]), [1, 2, 3])}};
    const outputs = {'z': {resource: await utils.createGPUBuffer(device, utils.sizeOfShape([3]))}};
    graph.compute(inputs, outputs);
    utils.checkValue(await utils.readbackGPUBuffer(device, utils.sizeOfShape([3]), outputs.z.resource), [1., 32., 729.]);
  });
  ...
  it('pow broadcast with 1d of [3] and 1d of [1]', async function() {
    ...
    const x = builder.input('x', {type: 'float32', dimensions: [3]});
    const y = builder.constant(
        {type: 'float32', dimensions: [1]}, {resource: await utils.createGPUBuffer(device, utils.sizeOfShape([1]), [2])});
    const z = builder.pow(x, y);
    const graph = builder.build({z});
    const inputs = {'x': {resource: await utils.createGPUBuffer(device, utils.sizeOfShape([3]), [1, 2, 3])}};
    const outputs = {'z': {resource: await utils.createGPUBuffer(device, utils.sizeOfShape([3]))}};
    graph.compute(inputs, outputs);
    utils.checkValue(await utils.readbackGPUBuffer(device, utils.sizeOfShape([3]), outputs.z.resource), [1., 4., 9.]);
  });
  ...
  it('pow broadcast with 2d of [2, 3] and 1d of [3]', async function() {
    ...
    const y = builder.constant(
        {type: 'float32', dimensions: [3]}, {resource: await utils.createGPUBuffer(device, utils.sizeOfShape([3]), [1, 2, 3])});
    const z = builder.pow(x, y);
    const graph = builder.build({z});
    const inputs = {'x': {resource: await utils.createGPUBuffer(device, utils.sizeOfShape([2, 3]), [1, 2, 3, 4, 5, 6])}};
    const outputs = {'z': {resource: await utils.createGPUBuffer(device, utils.sizeOfShape([2, 3]))}};
    graph.compute(inputs, outputs);
    utils.checkValue(await utils.readbackGPUBuffer(device, utils.sizeOfShape([2, 3]), outputs.z.resource), [1., 4., 27., 4., 25., 216.]);
  });
});
