...
describe('test instanceNormalization', function() {
  ...
  it('instanceNormalization default', async function() {
    ...
    const size = utils.sizeOfShape(inputShape);
    const inputs = {'input': {resource: await utils.createGPUBuffer(device, size, [-1, 0, 1, 2, 3, 4])}};
    const outputs = {'output': {resource: await utils.createGPUBuffer(device, size)}};
    graph.compute(inputs, outputs);
    const expected = [-1.2247356, 0., 1.2247356, -1.2247356, 0., 1.2247356];
    utils.checkValue(await utils.readbackGPUBuffer(device, size, outputs.output.resource), expected);
  });
  ...
  it('instanceNormalization with scale', async function() {
    ...
    const scale = builder.constant(desc, {resource: await utils.createGPUBuffer(device, utils.sizeOfShape([2]), [1.0, 1.5])});
  ...
  it('instanceNormalization with bias', async function() {
    ...
    const input =
        builder.input('input', {type: 'float32', dimensions: inputShape});
    const desc = {type: 'float32', dimensions: [2]};
    const bias = builder.constant(desc, {resource: await utils.createGPUBuffer(device, utils.sizeOfShape([2]), [0, 1])});
  ...
  it('batchNormalization with epsilon', async function() {
    ...
    const scale = builder.constant(
        desc, {resource: await utils.createGPUBuffer(device, utils.sizeOfShape([3]), [0.55290383, -1.1786512, -0.12353817])});
    const bias = builder.constant(
        desc, {resource: await utils.createGPUBuffer(device, utils.sizeOfShape([3]), [0.36079535, 2.3073995, -0.12267359])});
    const epsilon = 1e-2;
    const output = builder.instanceNormalization(input, {scale, bias, epsilon});
    const graph = builder.build({output});
    const size = utils.sizeOfShape(inputShape);
    const inputs = {
      'input': {resource: await utils.createGPUBuffer(device, size, [
